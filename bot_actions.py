...
COINS = [
    ...
    ("XRPUSDT",    "XRP/USDT",    "XRP"),     # Alta liquidez
    ("DOGEUSDT",   "DOGE/USDT",   "DOGE"),    # Meme coin líder
    ("PEPEUSDT",   "PEPE/USDT",   "PEPE"),    # Meme coin explosivo
    ("BONKUSDT",   "BONK/USDT",   "BONK"),    # Meme Solana
    ("JUPUSDT",    "JUP/USDT",    "JUP"),     # DEX Solana
    ("SEIUSDT",    "SEI/USDT",    "SEI"),     # L1 velocidade
    ("WIFUSDT",    "WIF/USDT",    "WIF"),     # Meme Solana top
    ("TIAUSDT",    "TIA/USDT",    "TIA"),     # Modular blockchain
    ("STRKUSDT",   "STRK/USDT",   "STRK"),    # L2 Ethereum (Starknet)
    ("ARBUSDT",    "ARB/USDT",    "ARB"),     # L2 Ethereum líder
    ("OPUSDT",     "OP/USDT",     "OP"),      # L2 Optimism
    ("INJUSDT",    "INJ/USDT",    "INJ"),     # DeFi Layer1
    ("RENDERUSDT", "RENDER/USDT", "RENDER"),  # AI + GPU
]
...
def analyze(sym, candles):
    ...
    e200 = ema(closes,       200)
    t_bull = price > e200 and e10 > e21 and e21 > e50
    t_bear = price < e200 and e10 < e21 and e21 < e50
    ...
    rsi_bull = 45 < rsi_v < 70
    rsi_bear = 30 < rsi_v < 55

    avg_vol  = sum(vols[-20:]) / 20
    v_strong = vols[-1] > avg_vol
    ...
    e200p  = ema(closes[:-5], 200)
    e200s  = 10 if e200 > e200p else -10
    pdi_sc = 10 if adx_ok else -10

    score = (
        (35 if t_bull else -35 if t_bear else 0) +
        (20 if m_bull else -20 if m_bear else 0) +
    ...
    sig = None
    if score > 30 and t_bull and m_bull and adx_ok and v_strong:
        sig = "LONG"
    elif score < -30 and t_bear and m_bear and adx_ok and v_strong:
        sig = "SHORT"

    return {
        "price": price, "score": score, "atr": atr_v,
        "rsi": rsi_v, "adx": adx_v,
        "trend": "BULL" if t_bull else "BEAR" if t_bear else "NEUTRO",
        "sig": sig,
    }
...
async def send_telegram(session, sym, label, short, sig_type, price, atr, score):
    ...
    body = {"chat_id": TG_CHATID, "text": text, "parse_mode": "MarkdownV2"}

    try:
        async with session.post(url, json=body, timeout=aiohttp.ClientTimeout(total=10)) as r:
            data = await r.json()
            if data.get("ok"):
                log.info(f"✅ Telegram → {sig_type} {short} | Score {score}")
            else:
...
async def main():
    if not TG_TOKEN or not TG_CHATID:
        log.error("❌ Configure TG_TOKEN e TG_CHATID nos Secrets do GitHub!")
        return
        ...
        for sym, label, short in COINS:
            ...
            if sig:
                ...
                if now - last_t >= COOLDOWN:
                    ...
                    signals_sent += 1
                    await send_telegram(
                        session, sym, label, short,
                        sig, result["price"], result["atr"], result["score"]
                    )
                else:
                    mins = int((COOLDOWN - (now - last_t)) / 60)
                    log.info(f"  ⏳ {short} em cooldown ({mins}min restantes)")
