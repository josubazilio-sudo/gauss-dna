"""
GAUSS+DNA ELITE + FLEX — Bot GitHub Actions
SIGNAL_MODE=ELITE: criterios completos Pine Script (sinais raros, precisos)
...
def analyze(sym, candles):
    ...
    # MACD
    ml,sl,hist,hist_p,hist_pp=macd_calc(closes)
    ha_close=(ml+sl+hist)/3
    ha_open=(ema_series(closes,12)[-2]-ema_series(closes,26)[-2]+ema_series(ema_series(closes,12)[-9:],9)[-2])/1 if n>27 else ml
    ha_bull=ha_close>ha_open
    macd_bull=ml>sl and hist>hist_p and hist>0 and ha_bull
    macd_bear=ml<sl and hist<hist_p and hist<0 and not ha_bull
    ...
    long_elite=(strong_trend and trend_bull and align_bull and e200_rising and
                macd_bull and f_bull and f_strong and adx_long_ok and rsi_bull and
                v_strong and not_ext_long and kalman_up and
                (bull_impulse or liq_long) and score>50)
    short_elite=(strong_trend and trend_bear and align_bear and e200_falling and
                 macd_bear and f_bear and f_strong and adx_short_ok and rsi_bear and
                 v_strong and not_ext_short and kalman_down and
                 (bear_impulse or liq_short) and score<-50)
    early_long=(adx_long_ok and v_strong and sell_exhaust and liq_long and
                bull_absorb and f_bull and trend_bull and e200_rising and kalman_up)
    early_short=(adx_short_ok and v_strong and buy_exhaust and liq_short and
                 bear_absorb and f_bear and trend_bear and e200_falling and kalman_down)

    # ── SINAIS FLEX ──
    long_flex=(score>30 and (trend_bull or kalman_up) and macd_bull and adx>15 and f_bull and v_strong)
    short_flex=(score<-30 and (trend_bear or kalman_down) and macd_bear and adx>15 and f_bear and v_strong)
    ...
    return {"price":price,"score":score,"atr":atr,"rsi":rsi,"adx":adx,
            "kalman_up":kalman_up,"trend":"BULL" if trend_bull else "BEAR" if trend_bear else "NEUTRO","sig":sig}
...
async def send_telegram(session, sym, label, short, sig_type, price, atr, score):
    ...
    try:
        async with session.post(url,json={"chat_id":TG_CHATID,"text":text,"parse_mode":"MarkdownV2"},
            ...
            data=await r.json()
            if data.get("ok"): log.info(f"✅ {sig_type} {short} Score:{score} [{SIGNAL_MODE}]")
...
async def main():
    ...
    log.info(f"🚀 GAUSS+DNA | Modo: {SIGNAL_MODE} | TF: {TIMEFRAME} | {len(COINS)} moedas")
    async with aiohttp.ClientSession() as session:
        for sym,label,short in COINS:
            ...
            log.info(f"{short:7s} | Score {result['score']:+4d} | RSI {result['rsi']:5.1f} | ADX {result['adx']:5.1f} | K:{'UP' if result['kalman_up'] else 'DN'} | {result['sig'] or '—'}")
            if result["sig"]:
                if now-last_sig.get(sym,0)>=COOLDOWN:
                    last_sig[sym]=now; sent+=1
                    await send_telegram(session,sym,label,short,result["sig"],result["price"],result["atr"],result["score"])
                else:
